...
import addressesReportTemplate from 'templates/views/addresses.htm';
export default ko.components.register('views/addresses', {
    viewModel: function(params) {
        ...
        const locations = places.
            ...
            map(place => {
                return $.ajax({
                    url: generateArchesURL("api_resource_report", {resourceid: place.link.split("/").pop()}),
                    type: 'GET',
                    dataType: 'json',
                    data: {
                        format: 'json',
        ...
        let addresses = atAddresses?.
            ...
            map(atAddress => {
                return $.ajax({
                    url: generateArchesURL("api_resource_report", {resourceid: atAddress.link.split("/").pop()}),
                    type: 'GET',
                    dataType: 'json',
                    data: {
                        format: 'json',
                    ...
                    then(data => {
                        ...
                        if (occupiedAddress && occupiedAddress.data[data.report_json["Occupied Address"]["@node_id"]].length) {
                            ...
                            return Promise.all([
                                ...
                                $.ajax({
                                    url: generateArchesURL("api_resource_report", {resourceid: addressId}),
                                    type: 'GET',
                                    dataType: 'json',
                                    data: {
                                        format: 'json',
